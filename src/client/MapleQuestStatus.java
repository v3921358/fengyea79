...
public class MapleQuestStatus implements Serializable
{
    ...
    public final boolean mobKilled(final int id, final int skillID) {
        try {
            ...
            final Integer mob = Integer.valueOf(this.killedMobs.get(Integer.valueOf(id)));
            if (mob != null) {
                final int mo = this.maxMob(id);
                if ((int)mob >= mo) {
                    return false;
                }
                this.killedMobs.put(Integer.valueOf(id), Math.min((int)mob + 1, mo));
                return true;
            }
            else {
                for (final Entry<Integer, Integer> mo2 : this.killedMobs.entrySet()) {
                    if (this.questCount((int)Integer.valueOf(mo2.getKey()), id)) {
                        ...
                        this.killedMobs.put(mo2.getKey(), Math.min((int)Integer.valueOf(mo2.getValue()) + 1, mobb));
                        return true;
                    }
                }
            }
