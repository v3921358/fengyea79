    ...
    public static void dropHelp(final MapleClient c, final int type) {
        ...
        for (int i = 0; i <= check; ++i) {
            if (commandList.containsKey(Integer.valueOf(i))) {
                ...
                for (final String s : (ArrayList<String>)commandList.get(Integer.valueOf(i))) {
        ...
        if (c != null) {
            ...
            if (commandPrefix == PlayerGMRank.普通玩家.getCommandPrefix()) {
                ...
                if (co == null || co.getType() != type) {
                    sendDisplayMessage(c, "沒有這個指令,可以使用 @帮助/@help 來查看指令.", type);
                    return true;
                }
                try {
                ...
                final int 游戏指令开关 = (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("游戏指令开关"));
                if (游戏指令开关 > 0) {
                    if (c.getPlayer().getGMLevel() == PlayerGMRank.神.getLevel()) {
                        ...
                        if (show.size() == 1 && !splitted2[0].equals(show.get(0))) {
                            sendDisplayMessage(c, "自动识別关联指令[" + (String)show.get(0) + "].", type);
                            splitted2[0] = (String)show.get(0);
                        }
                        if (line.charAt(0) == '*') {
                            final CommandObject co2 = (CommandObject)CommandProcessor.commands.get(splitted2[0]);
                            if (co2 == null || co2.getType() != type) {
                                if (splitted2[0].equals(line.charAt(0) + "help")) {
                                    dropHelp(c, 0);
                                    return true;
                                }
    ...
    private static void ShowMsg(final MapleClient c, final String line, final CommandType type) {
        if (c.getPlayer() != null) {
            ...
            if (c.getPlayer().getGMLevel() == 5) {
                ...
                System.out.println("＜老练习生＞ " + c.getPlayer().getName() + " 使用了指令: " + line);
            }
            else if (c.getPlayer().getGMLevel() == 1) {
                System.out.println("＜新练习生＞ " + c.getPlayer().getName() + " 使用了指令: " + line);
            }
            else if (c.getPlayer().getGMLevel() != 88) {
