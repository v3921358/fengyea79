...
public class MapleCharacter extends AbstractAnimatedMapleMapObject implements Serializable
{
        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            try {
                ...
                if (channelserver) {
                    ...
                    while (rs.next()) {
                        keyb.put(rs.getInt("keye"), new Pair<Byte, Integer>(rs.getByte("type"), rs.getInt("action")));
                    }
                    rs.close();
                    ps.close();
    ...
    public void registerEffect(final MapleStatEffect effect, final long starttime, final ScheduledFuture<?> schedule, final List<Pair<MapleBuffStat, Integer>> statups, final boolean silent, final int localDuration, final int cid) {
        ...
        for (final Pair<MapleBuffStat, Integer> statup : statups) {
            ...
            this.skillID.put(effect.getSourceId(), mbsvh);
        }
        if (clonez > 0) {
    ...
    private boolean deregisterBuffStats(final List<MapleBuffStat> stats) {
        ...
        for (final MapleBuffStat stat : stats) {
            ...
            if (mbsvh != null) {
                this.skillID.remove(mbsvh.effect.getSourceId());
    ...
    public List<PlayerBuffValueHolder> getAllBuffs() {
        ...
        for (final Entry<MapleBuffStat, MapleBuffStatValueHolder> mbsvh : allBuffs) {
            final Pair<Integer, Byte> key = new Pair<Integer, Byte>(((MapleBuffStatValueHolder)mbsvh.getValue()).effect.getSourceId(), ((MapleBuffStatValueHolder)mbsvh.getValue()).effect.getLevel());
            if (alreadyDone.containsKey(key)) {
                ((PlayerBuffValueHolder)ret.get((int)Integer.valueOf(alreadyDone.get(key)))).statup.add(new Pair<MapleBuffStat, Integer>(mbsvh.getKey(), Integer.valueOf(((MapleBuffStatValueHolder)mbsvh.getValue()).value)));
            }
            else {
    ...
    public void gainExp(final int total, final boolean show, final boolean inChat, final boolean white) {
        try {
            ...
            final int 骑士团等级上限 = (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("骑士团等级上限"));
            final int 冒险家等级上限 = (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("冒险家等级上限"));
            if (total > 0) {
    ...
    public void DoLevelMsg() {
        final int 升级快讯 = (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("升级快讯开关"));
    ...
    public void gainmoneyb(final int slot) {
        try (final Connection con = DatabaseConnection.getConnection()) {
            ...
            System.err.println("gainmoneyb" + ex);
    ...
    public void giveDebuff(final MapleDisease disease, final int x, final long duration, final int skillid, final int level) {
        ...
        if (!this.hasDisease(disease) && this.diseases.size() < 2) {
            ...
            if (this.F().get(FumoSkill.FM("异常抗性")) != null) {
                ...
                final int 免疫 = (int)Integer.valueOf(this.F().get(FumoSkill.FM("异常抗性")));
                if (r1 <= (double)免疫) {
            ...
            if (this.F().get(FumoSkill.FM("异常抗性")) != null) {
                int 抗性 = (int)Integer.valueOf(this.F().get(FumoSkill.FM("异常抗性")));
                if (抗性 > 65) {
                    抗性 = 65;
                }
    ...
    public int getBossLog(final String boss, final int type) {
        ...
        try {
            ...
            DatabaseConnection.cleanUP(rs, ps, con);
        }
    }
    ...
    public void cancelFairySchedule(final boolean exp) {
        ...
        if (exp) {
            final int 精灵吊坠 = (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("精灵吊坠经验加成"));
            this.fairyExp = (byte)精灵吊坠;
        }
    }
    
    public void 人气经验加成() {
        final int 人气 = this.getFame() * (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("人气经验加成"));
    ...
    public void addMoveMob(final int mobid) {
        if (this.movedMobs.containsKey(Integer.valueOf(mobid))) {
            this.movedMobs.put(Integer.valueOf(mobid), Integer.valueOf((int)Integer.valueOf(this.movedMobs.get(Integer.valueOf(mobid))) + 1));
            if ((int)Integer.valueOf(this.movedMobs.get(Integer.valueOf(mobid))) > 30) {
    ...
    public static void setMP(final Map<MapleCharacter, Integer> GiveList, final boolean showMessage) {
        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            while (iter.hasNext()) {
                ...
                final int MP = (int)Integer.valueOf(GiveList.get(chr));
    ...
    public int getRMB() {
        ...
        try {
            ...
            try {
                ...
                System.err.println("获取角色rmb失败。" + ex2);
            }
        }
        return point;
    }
    ...
    public List<Integer> getiApprentice(final int charaid) {
        ...
        try {
            ...
            final ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                charid.add(rs.getInt("characterid"));
            }
            rs.close();
            ps.close();
            return charid;
    ...
    public void updateOfflineTime3(final int x, final int y, final int chairid, final long lefttime) {
        ...
        try {
            ...
            System.out.println("结束客户端保存离线数据错误!" + Ex);
        }
        finally {
            try {
                if (con1 != null) {
                ...
                System.out.println("结束客户端保存离线数据自动关闭数据库异常" + Ex2);
            }
        }
    }
    ...
    public int getLastOfflineTime2() {
        ...
        try {
            ...
            System.out.println("离线挂机数据保存异常" + Ex);
        }
        finally {
            try {
                if (conn != null) {
