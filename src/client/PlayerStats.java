    ...
    public void recalcLocalStats(final boolean first_login) {
        ...
        for (final IItem item : chra.getInventory(MapleInventoryType.EQUIPPED)) {
            ...
            if (equip.getPosition() == -11 && GameConstants.isMagicWeapon(equip.getItemId())) {
                ...
                this.element_fire = (int)Integer.valueOf(eqstat.get("incRMAF"));
                this.element_ice = (int)Integer.valueOf(eqstat.get("incRMAI"));
                this.element_light = (int)Integer.valueOf(eqstat.get("incRMAL"));
                this.element_psn = (int)Integer.valueOf(eqstat.get("incRMAS"));
                this.def = (int)Integer.valueOf(eqstat.get("elemDefault"));
            }
            ...
            if (set > 0) {
                int value = 1;
                if (this.setHandling.get(Integer.valueOf(set)) != null) {
                    value += (int)Integer.valueOf(this.setHandling.get(Integer.valueOf(set)));
                }
        ...
        for (final IItem item2 : chra.getInventory(MapleInventoryType.CASH)) {
            ...
            if (this.dropMod == 1) {
                ...
                else if (item2.getItemId() == 5360000 && hour >= 0 && hour <= 24) {
                    this.dropMod = 2;
                    this.金币爆率 = 1;
                }
            ...
            if (eq.getEquipLevel() > lvlz) {
                for (int i = eq.getEquipLevel() - lvlz; i > 0; --i) {
                    ...
                    if (inc != null && inc.containsKey(Integer.valueOf(lvlz + i))) {
                        eq = ii.levelUpEquip(eq, (Map<String, Integer>)inc.get(Integer.valueOf(lvlz + i)));
                    }
                    if (GameConstants.getStatFromWeapon(eq.getItemId()) == null) {
                        final Map<Integer, List<Integer>> ins = ii.getEquipSkills(eq.getItemId());
                        if (ins != null && ins.containsKey(Integer.valueOf(lvlz + i))) {
                            for (final Integer z : (List<Integer>)ins.get(Integer.valueOf(lvlz + i))) {
                                if (Math.random() < 0.1) {
