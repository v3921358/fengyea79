...
public class EventManager
{
    ...
    public void startInstance_CharID(final MapleCharacter character) {
        try {
            final EventInstanceManager eim = (EventInstanceManager)(EventInstanceManager)this.iv.invokeFunction("setup", character.getId());
            eim.registerPlayer(character);
        }
        catch (ScriptException | NoSuchMethodException ex) {
    ...
    public void startInstance(final MapleParty party, final MapleMap map) {
        try {
            final EventInstanceManager eim = (EventInstanceManager)(EventInstanceManager)this.iv.invokeFunction("setup", party.getId());
    ...
    public void startInstance(final MapleSquad squad, final MapleMap map, final int questID) {
        ...
        if (!squad.getLeader().isGM() && this.name.equals("CWKPQ") && squad.getJobs().size() < 5) {
