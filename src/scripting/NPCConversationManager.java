...
public class NPCConversationManager extends AbstractPlayerInteraction
{
    ...
    public void getMobs(final int itemid) {
        ...
        for (int i = 0; i < mobs.size(); ++i) {
            ...
            text = text + "#r#o" + mobs.get(i) + "##k " + ((quest > 0 && MapleQuest.getInstance(quest).getName().length() > 0) ? ("#b需要進行 " + MapleQuest.getInstance(quest).getName() + " 任務來取得#k") : "") + "\r\n";
        }
        this.sendNext(text);
    }
    ...
    public String 等级排行榜() {
        ...
        try {
            ...
            final ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                if (rs.getInt("level") < (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("冒险家等级上限")) && rs.getInt("level") > 30) {
                    if (名次 < 10) {
    ...
    public String 满级排行榜() {
        ...
        try {
            ...
            final ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                if (rs.getInt("level") == (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("冒险家等级上限"))) {
                    final String 玩家名字 = rs.getString("name");
                    final String 职业 = this.职业(rs.getInt("job"));
    ...
    public void gainmoneym(final int cid, final int slot) {
        ...
        FileoutputUtil.logToFile("日志/商人交易系统/商人利益点更改.txt", " 更改值(" + ((this.c.getPlayer().getAccountID() == cid) ? "自身" : Integer.valueOf(cid)) + ") " + slot + "");
    }
    ...
    public void setmoneym(final int cid, final int slot) {
        ...
        FileoutputUtil.logToFile("日志/商人交易系统/商人利益点更改.txt", " 设置值(" + ((this.c.getPlayer().getAccountID() == cid) ? "自身" : Integer.valueOf(cid)) + ") " + slot + "");
    }
