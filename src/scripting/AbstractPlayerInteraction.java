...
public abstract class AbstractPlayerInteraction
{
    ...
    public int getTotalOnline() {
        return (int)Integer.valueOf(ChannelServer.getAllInstances().stream().map(cserv -> cserv.getConnectedClients()).reduce(Integer.valueOf(0), Integer::sum));
    }
    ...
    public List<ArrayList> getSevenDayPayLog(final int day) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (diffday < day) {
                    ret.set(diffday, Integer.valueOf((int)Integer.valueOf((int)ret.get(diffday)) + rmb));
                }
            }
            ps.close();
            rs.close();
