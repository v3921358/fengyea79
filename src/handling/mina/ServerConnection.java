...
public class ServerConnection
{
    ...
    public void run() {
        try {
            this.boot = ((ServerBootstrap)((ServerBootstrap)new ServerBootstrap().group(this.bossGroup, this.workerGroup).channel(NioServerSocketChannel.class)).option(ChannelOption.SO_BACKLOG, Integer.valueOf(WorldConstants.USER_LIMIT))).childOption(ChannelOption.TCP_NODELAY, Boolean.valueOf(true)).childOption(ChannelOption.SO_KEEPALIVE, Boolean.valueOf(true)).childHandler((ChannelHandler)new ServerInitializer(this.world, this.channels));
            try {
