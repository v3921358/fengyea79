...
public class NPCHandler
{
        ...
        for (final MapleInventoryType type : array) {
            for (final IItem item : c.getPlayer().getInventory(type)) {
                if (item instanceof Equip) {
                    ...
                    if ((int)Integer.valueOf(eqStats.get("durability")) <= 0 || eq.getDurability() >= (int)Integer.valueOf(eqStats.get("durability"))) {
                        continue;
                    }
                    final double rPercentage = 100.0 - Math.ceil((double)eq.getDurability() * 1000.0 / ((double)(int)Integer.valueOf(eqStats.get("durability")) * 10.0));
    ...
    public static final void repair(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (eq.getDurability() < 0 || (int)Integer.valueOf(eqStats.get("durability")) <= 0 || eq.getDurability() >= (int)Integer.valueOf(eqStats.get("durability"))) {
            return;
        }
        final double rPercentage = 100.0 - Math.ceil((double)eq.getDurability() * 1000.0 / ((double)(int)Integer.valueOf(eqStats.get("durability")) * 10.0));
        ...
        eq.setDurability((int)Integer.valueOf(eqStats.get("durability")));
    ...
    public static final void UseItemQuest(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        for (final IItem i : c.getPlayer().getInventory(MapleInventoryType.ETC)) {
            if (i.getItemId() / 10000 == 422) {
                ...
                if (questItemInfo != null && (int)Integer.valueOf(questItemInfo.getLeft()) == qid && ((List<Integer>)questItemInfo.getRight()).contains(Integer.valueOf(itemId))) {
                    found = true;
                    break;
                }
                continue;
