...
public class ItemMakerHandler
{
    ...
    private static void addEnchantStats(final Map<String, Byte> stats, final Equip item) {
        int s = (byte)Byte.valueOf(stats.get("incPAD"));
        if (s != 0) {
            item.setWatk((short)(item.getWatk() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incMAD"));
        if (s != 0) {
            item.setMatk((short)(item.getMatk() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incACC"));
        if (s != 0) {
            item.setAcc((short)(item.getAcc() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incEVA"));
        if (s != 0) {
            item.setAvoid((short)(item.getAvoid() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incSpeed"));
        if (s != 0) {
            item.setSpeed((short)(item.getSpeed() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incJump"));
        if (s != 0) {
            item.setJump((short)(item.getJump() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incMaxHP"));
        if (s != 0) {
            item.setHp((short)(item.getHp() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incMaxMP"));
        if (s != 0) {
            item.setMp((short)(item.getMp() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incSTR"));
        if (s != 0) {
            item.setStr((short)(item.getStr() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incDEX"));
        if (s != 0) {
            item.setDex((short)(item.getDex() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incINT"));
        if (s != 0) {
            item.setInt((short)(item.getInt() + s));
        }
        s = (byte)Byte.valueOf(stats.get("incLUK"));
        if (s != 0) {
            item.setLuk((short)(item.getLuk() + s));
        }
        s = (byte)Byte.valueOf(stats.get("randOption"));
        if (s > 0) {
        ...
        s = (byte)Byte.valueOf(stats.get("randStat"));
        if (s > 0) {
            final boolean success = Randomizer.nextBoolean();
    ...
    private static int getRandomGem(final List<Pair<Integer, Integer>> rewards) {
        ...
        for (final Pair<Integer, Integer> p : rewards) {
            final int itemid = (int)Integer.valueOf(p.getLeft());
            for (int i = 0; i < (int)Integer.valueOf(p.getRight()); ++i) {
