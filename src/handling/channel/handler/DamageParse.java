                    ...
                    if (!GameConstants.isElseSkill(attack.skill)) {
                        ...
                        if (GameConstants.isAran((int)player.getJob())) {
                            if (player.getLevel() > 10 && (int)eachd > atk && (double)(int)eachd > maxDamagePerHit * 2.0) {
                                FileoutputUtil.logToFile("logs/Hack/伤害異常_狂狼.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + (int)player.getLevel() + ">: " + player.getName() + " 技能代码: " + attack.skill + " 怪物: " + monster.getId() + " 最高伤害: " + atk + " 本次伤害 :" + eachd + " 預計伤害: " + (int)maxDamagePerHit + "是否為BOSS: " + monster.getStats().isBoss());
                            }
                ...
                switch (attack.skill) {
                    ...
                    case 14111005: {
                        if (player.hasBuffedValue(MapleBuffStat.WK_CHARGE) && !monster.getStats().isBoss()) {
                            ...
                            if (eff != null) {
                                monster.applyStatus(player, new MonsterStatusEffect(MonsterStatus.SPEED, eff.getX(), eff.getSourceId(), null, false), false, (long)(eff.getY() * 1000), monster.getStats().isBoss(), eff);
                            }
                        }
                ...
                if (totDamageToOneMonster > 0) {
                    ...
                    if (player.hasBuffedValue(MapleBuffStat.BLIND)) {
                        ...
                        if (eff2 != null && eff2.makeChanceResult()) {
                            final MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.ACC, eff2.getX(), eff2.getSourceId(), null, false);
                    ...
                    if (player.hasBuffedValue(MapleBuffStat.HAMSTRING)) {
                        ...
                        if (eff2 != null && eff2.makeChanceResult()) {
                            final MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.SPEED, eff2.getX(), 3121007, null, false);
                            monster.applyStatus(player, monsterStatusEffect, false, (long)(eff2.getY() * 1000), monster.getStats().isBoss(), eff2);
                        }
                    }
    ...
    public static final AttackInfo parseExplosionAttack(final LittleEndianAccessor lea, final AttackInfo ret) {
        ...
        for (int i = 0; i < ret.targets; ++i) {
            ...
            for (int k = 0; k < bullets2; ++k) {
                allDamageNumbers.add(new Pair<Integer, Boolean>(lea.readInt(), Boolean.valueOf(false)));
            }
            ret.allDamage.add(new AttackPair(oid, allDamageNumbers));
            lea.skip(4);
        }
