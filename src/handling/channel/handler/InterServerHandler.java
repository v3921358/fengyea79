...
public class InterServerHandler
{
    ...
    public static final void LoggedIn(final int playerid, final MapleClient c) {
        ...
        final int 管理隐身 = (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("管理隐身开关"));
        if (管理隐身 <= 0 && player.isGM()) {
            SkillFactory.getSkill(9001004).getEffect(1).applyTo(player);
            final int 管理加速 = (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("管理加速开关"));
            if (管理加速 <= 0) {
        ...
        if ((int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("上线提醒开关")) <= 0) {
        ...
        if ((int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("登陆帮助开关")) == 0) {
            if (player.getGMLevel() > 0 && player.getBossLog("管理上线提示") == 0) {
                player.dropMessage(5, "指令: [在枫叶服务端内置] 查看管理员指令文本");
        ...
        if ((int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("幸运职业开关")) == 0) {
    ...
    static {
        InterServerHandler.离线挂机 = Boolean.parseBoolean(ServerProperties.getProperty("FengYeDuan.离线挂机"));
    }
}
