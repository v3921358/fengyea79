...
public class MapleGuildRanking
{
    ...
    public List<JobRankingInfo> getJobRank(final int type) {
        if (this.JobRanks.get(Integer.valueOf(type)) == null || ((List<JobRankingInfo>)this.JobRanks.get(Integer.valueOf(type))).isEmpty()) {
            this.loadJobRank(type);
        }
        return (List<JobRankingInfo>)this.JobRanks.get(Integer.valueOf(type));
    }
    
    private void loadJobRank(final int type) {
        if (this.JobRanks.get(Integer.valueOf(type)) != null) {
            ((List<JobRankingInfo>)this.JobRanks.get(Integer.valueOf(type))).clear();
        }
        String jobRange = "";
        if (type == 1) {
