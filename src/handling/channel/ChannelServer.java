...
public class ChannelServer implements Serializable
{
    ...
    private ChannelServer(final int channel) {
        this.port = (short)Integer.parseInt(ChannelServer.cf.getConfig("FengYeDuan.channel.port1"));
    ...
    public final void setup() {
        ...
        try {
            this.eventSM = new EventScriptManager(this, ServerProperties.getProperty("FengYeDuan.events").split(","));
            this.port = (short)(ServerProperties.getProperty("FengYeDuan.channel.port", ChannelServer.DEFAULT_PORT) + this.channel - 1);
        }
        catch (Exception e) {
    ...
    public void closeAllMerchants() {
        ...
        try {
            final Iterator hmit = this.merchants.entrySet().iterator();
            while (hmit.hasNext()) {
                ((HiredMerchant)((Entry<Integer, HiredMerchant>)hmit.next()).getValue()).closeShop(true, false);
                hmit.remove();
                ++ret;
            }
        }
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        if ((int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("滚动公告开关")) <= 0) {
    ...
    public final void reloadEvents() {
        this.eventSM.cancel();
        (this.eventSM = new EventScriptManager(this, ServerProperties.getProperty("FengYeDuan.events").split(","))).init();
    }
    ...
    public int getLastOfflineTime2() {
        ...
        try {
            ...
            try {
                ...
                System.out.println("离线挂机数据保存自动关闭数据库异常" + Ex2);
            }
        }
        return retnumber;
    }
    ...
    static {
        ChannelServer.离线挂机 = Boolean.parseBoolean(ServerProperties.getProperty("FengYeDuan.离线挂机"));
        ChannelServer.cf = new Config();
        DEFAULT_PORT = (short)Integer.parseInt(ChannelServer.cf.getConfig("FengYeDuan.channel.port1"));
