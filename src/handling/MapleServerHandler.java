...
public class MapleServerHandler extends ChannelInboundHandlerAdapter
{
    ...
    public void channelActive(final ChannelHandlerContext ctx) throws Exception {
        ...
        final Pair<Long, Byte> track = (Pair<Long, Byte>)this.tracker.get(address);
        byte count;
        if (track == null) {
            count = 1;
        }
        ...
        this.tracker.put(address, new Pair<Long, Byte>(System.currentTimeMillis(), Byte.valueOf(count)));
        if (this.channel == -10) {
        ...
        while (i < length) {
            ...
            if (recv.getValue() == header_num) {
                ...
                if (c.getPlayer() != null && c.isMonitored() && !MapleServerHandler.blocked.contains(recv)) {
                    FilePrinter.print("Monitored/" + c.getPlayer().getName() + ".txt", String.valueOf(recv) + " (" + Integer.toHexString((int)header_num) + ") Handled: \r\n" + slea.toString() + "\r\n");
                }
                try {
