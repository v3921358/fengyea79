...
public class MapleGuild implements Serializable
{
        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            while (rs.next()) {
                ...
                try (final PreparedStatement pse = con.prepareStatement("SELECT * FROM bbs_replies WHERE threadid = ?")) {
                    ...
                    while (rse.next()) {
                        thread.replies.put(thread.replies.size(), new handling.world.guild.MapleBBSThread.MapleBBSReply(thread.replies.size(), rse.getInt("postercid"), rse.getString("content"), rse.getLong("timestamp")));
                    }
                    rse.close();
                }
                    ...
                    try (final ResultSet rs = ps.getGeneratedKeys()) {
                        if (!rs.next()) {
                            ...
                            try (final PreparedStatement pse = con.prepareStatement("INSERT INTO bbs_replies (`threadid`, `postercid`, `timestamp`, `content`, `guildid`) VALUES (?, ?, ?, ?, ?)")) {
                                ...
                                for (final handling.world.guild.MapleBBSThread.MapleBBSReply r : bb.replies.values()) {
    ...
    public final List<MapleBBSThread> getBBS() {
        ...
        Collections.sort(ret, (Comparator<? super MapleBBSThread>)new handling.world.guild.MapleBBSThread.ThreadComparator());
        return ret;
    }
    ...
    public final int addBBSThread(final String title, final String text, final int icon, final boolean bNotice, final int posterID) {
        final int add = (this.bbs.get(Integer.valueOf(0)) == null) ? 1 : 0;
    ...
    public final void editBBSThread(final int localthreadid, final String title, final String text, final int icon, final int posterID, final int guildRank) {
        final MapleBBSThread thread = (MapleBBSThread)this.bbs.get(Integer.valueOf(localthreadid));
    ...
    public final void addBBSReply(final int localthreadid, final String text, final int posterID) {
        final MapleBBSThread thread = (MapleBBSThread)this.bbs.get(Integer.valueOf(localthreadid));
        if (thread != null) {
            thread.replies.put(thread.replies.size(), new handling.world.guild.MapleBBSThread.MapleBBSReply(thread.replies.size(), posterID, text, System.currentTimeMillis()));
        }
    }
    ...
    public final void deleteBBSReply(final int localthreadid, final int replyid, final int posterID, final int guildRank) {
        final MapleBBSThread thread = (MapleBBSThread)this.bbs.get(Integer.valueOf(localthreadid));
        if (thread != null) {
            final handling.world.guild.MapleBBSThread.MapleBBSReply reply = (handling.world.guild.MapleBBSThread.MapleBBSReply)thread.replies.get(Integer.valueOf(replyid));
