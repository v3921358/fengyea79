    ...
    public MapleFamily(final int fid) {
        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            while (rs.next()) {
                ...
                this.members.put(rs.getInt("id"), new MapleFamilyCharacter(rs.getInt("id"), (int)rs.getShort("level"), rs.getString("name"), -1, rs.getInt("job"), fid, rs.getInt("seniorid"), rs.getInt("junior1"), rs.getInt("junior2"), rs.getInt("currentrep"), rs.getInt("totalrep"), false));
            }
            rs.close();
            ps.close();
