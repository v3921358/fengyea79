...
public class World
{
    ...
    public static Map<Integer, Integer> getConnected() {
        ...
        for (final ChannelServer cs : ChannelServer.getAllInstances()) {
            ...
            ret.put(cs.getChannel(), Integer.valueOf(curConnected));
            total += curConnected;
        }
            ...
            public void run() {
                ...
                if (rate.equals("经验")) {
                    for (final ChannelServer cservs : ChannelServer.getAllInstances()) {
                        cservs.setExpRate(Integer.parseInt(ServerProperties.getProperty("FengYeDuan.expRate")));
                        cservs.broadcastPacket(MaplePacketCreator.serverNotice(6, "[系统公告]：经验倍率活动已经结束，已经恢复正常值。"));
                    }
                }
                else if (rate.equals("爆率")) {
                    for (final ChannelServer cservs : ChannelServer.getAllInstances()) {
                        cservs.setDropRate(Integer.parseInt(ServerProperties.getProperty("FengYeDuan.dropRate")));
                        cservs.broadcastPacket(MaplePacketCreator.serverNotice(6, "[系统公告]：爆物倍率活动已经结束，已经恢复正常值。"));
                    }
                }
                else if (rate.equals("金币")) {
                    for (final ChannelServer cservs : ChannelServer.getAllInstances()) {
                        cservs.setMesoRate(Integer.parseInt(ServerProperties.getProperty("FengYeDuan.mesoRate")));
        ...
        if (numTimes % 100 == 0) {
            for (final MaplePet pet : chr.getSummonedPets()) {
                if (pet.getSummoned()) {
                    ...
                    final int vip道具 = 3994765;
                    if (chr.getItemQuantity(vip道具, true) > 0) {
                        newFullness = 100;
                    }
                    if (newFullness <= 5) {
                        pet.setFullness(15);
    ...
    public static class Party
    {
        ...
        public static MapleParty getParty(final int partyid) {
            return (MapleParty)Party.parties.get(Integer.valueOf(partyid));
        }
        
        public static MapleParty disbandParty(final int partyid) {
            return (MapleParty)Party.parties.remove(Integer.valueOf(partyid));
        }
    ...
    public static class Messenger
    {
        ...
        public static MapleMessenger getMessenger(final int messengerid) {
            return (MapleMessenger)Messenger.messengers.get(Integer.valueOf(messengerid));
        }
    ...
    public static class Guild
    {
        ...
        public static MapleGuild getGuild(final int id) {
            ...
            try {
                ret = (MapleGuild)Guild.guilds.get(Integer.valueOf(id));
            }
            finally {
        ...
        public static void disbandGuild(final int gid) {
            ...
            try {
                if (g != null) {
                    g.disbandGuild();
                    Guild.guilds.remove(Integer.valueOf(gid));
                }
            }
            finally {
                Guild.lock.writeLock().unlock();
    ...
    public static class Find
    {
        ...
        public static void forceDeregister(final int id) {
            Find.lock.writeLock().lock();
            try {
                Find.idToChannel.remove(Integer.valueOf(id));
            }
            finally {
                Find.lock.writeLock().unlock();
            }
        ...
        public static CharacterIdChannelPair[] multiBuddyFind(final int charIdFrom, final Collection<Integer> characterIds) {
            ...
            for (final Integer i : characterIds) {
                final Integer channel = findChannel((int)i);
                if ((int)channel > 0) {
    ...
    public static class Alliance
    {
        ...
        public static MapleGuildAlliance getAlliance(final int allianceid) {
            ...
            try {
                ret = (MapleGuildAlliance)Alliance.alliances.get(Integer.valueOf(allianceid));
            }
            finally {
        ...
        public static void setOldAlliance(final int gid, final boolean expelled, final int allianceid) {
            ...
            if (gid == -1) {
                Alliance.lock.writeLock().lock();
                try {
                    Alliance.alliances.remove(Integer.valueOf(allianceid));
                }
                finally {
                    Alliance.lock.writeLock().unlock();
                }
    ...
    public static class Family
    {
        ...
        public static MapleFamily getFamily(final int id) {
            ...
            try {
                ret = (MapleFamily)Family.families.get(Integer.valueOf(id));
            }
            finally {
        ...
        public static void disbandFamily(final int gid) {
            ...
            try {
                if (g != null) {
                    g.disbandFamily();
                    Family.families.remove(Integer.valueOf(gid));
                }
            }
            finally {
                Family.lock.writeLock().unlock();
