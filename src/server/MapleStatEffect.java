...
public class MapleStatEffect implements Serializable
{
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, final int newDuration, final boolean dc) {
        ...
        if (this.expinc != 0) {
            ...
            final server.MapleCarnivalFactory.MCSkill skil = MapleCarnivalFactory.getInstance().getSkill(this.nuffSkill);
            if (skil != null) {
    ...
    public final void silentApplyBuff(final MapleCharacter chr, final long starttime, final int localDuration, final List<Pair<MapleBuffStat, Integer>> statup, final int cid) {
        chr.registerEffect(this, starttime, server.Timer.BuffTimer.getInstance().schedule((Runnable)new CancelEffectAction(chr, this, starttime), starttime + (long)localDuration - System.currentTimeMillis()), statup, true, localDuration, cid);
    ...
    public final void applyEnergyBuff(final MapleCharacter applyto, final boolean infinity) {
        ...
        if (infinity) {
            ...
            final ScheduledFuture<?> schedule = server.Timer.BuffTimer.getInstance().schedule((Runnable)cancelAction, starttime + (long)this.duration - System.currentTimeMillis());
    ...
    private void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final int newDuration, final boolean dc) {
        ...
        switch (this.sourceid) {
            ...
            default: {
                if (this.isMorph() || this.isPirateMorph()) {
                    ...
                    Selfstat.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.MORPH, this.getMorph(applyto)));
                    break;
                }
                if (this.isMonsterRiding()) {
                    ...
                    if (mountid != 0 && mountid2 != 0) {
                        ...
                        applyto.getClient().getSession().writeAndFlush(MaplePacketCreator.giveMount(mountid2, this.sourceid, stat2));
        ...
        final CancelEffectAction cancelAction = new CancelEffectAction(applyto, this, starttime);
        try {
            final ScheduledFuture<?> schedule = server.Timer.BuffTimer.getInstance().schedule((Runnable)cancelAction, starttime + (long)localDuration - System.currentTimeMillis());
