...
public class AutobanManager implements Runnable
{
    ...
    public final void addPoints(final MapleClient c, final int points, final long expiration, final String reason) {
        ...
        try {
            ...
            if (this.points.containsKey(Integer.valueOf(acc))) {
                final int SavedPoints = (int)Integer.valueOf(this.points.get(Integer.valueOf(acc)));
                if (SavedPoints >= 5000) {
                    return;
                }
                this.points.put(Integer.valueOf(acc), Integer.valueOf(SavedPoints + points));
                final List<String> reasonList = (List<String>)this.reasons.get(Integer.valueOf(acc));
            ...
            if ((int)Integer.valueOf(this.points.get(Integer.valueOf(acc))) >= 5000) {
                if (c.getPlayer().isGM() || c.getPlayer().isClone()) {
                    c.getPlayer().dropMessage(5, "[警告] A/b 觸發 原因 : " + reason);
                    return;
                }
                ...
                sb.append(c.getMacs());
                sb.append("): ");
                for (final String s : (List<String>)this.reasons.get(Integer.valueOf(acc))) {
                    sb.append(s);
                    sb.append(", ");
                }
            }
    ...
    public final void run() {
        ...
        for (final ExpirationEntry e : this.expirations) {
            ...
            this.points.put(Integer.valueOf(e.acc), Integer.valueOf((int)Integer.valueOf(this.points.get(Integer.valueOf(e.acc))) - e.points));
        }
    }
