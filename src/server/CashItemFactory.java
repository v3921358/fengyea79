...
public class CashItemFactory {
    ...
    protected CashItemFactory() {
        ...
        this.itemMods = new HashMap<Integer, server.CashItemInfo.CashModInfo>();
    ...
    public void initialize() {
        ...
        for (final MapleData field : this.data.getData("Commodity.img").getChildren()) {
            ...
            if (SN > 0) {
                ...
                this.itemIdToSN.put(stats.getId(), Integer.valueOf(SN));
            }
            if (itemId > 0) {
    ...
    public final server.CashItemInfo.CashModInfo getModInfo(final int sn) {
        ...
        if (ret == null) {
            ...
            try (final Connection con = (Connection) DBConPool.getInstance().getDataSource().getConnection();
                ...
                if (rs.next()) {
                    ret = new server.CashItemInfo.CashModInfo(sn, rs.getInt("discount_price"), rs.getInt("mark"), rs.getInt("showup") > 0, rs.getInt("itemid"), rs.getInt("priority"), rs.getInt("package") > 0, rs.getInt("period"), rs.getInt("gender"), rs.getInt("count"), rs.getInt("meso"), rs.getInt("unk_1"), rs.getInt("unk_2"), rs.getInt("unk_3"), rs.getInt("extra_flags"), rs.getInt("mod"));
