...
public class MapleMonster extends AbstractLoadedMapleLife
{
    ...
    private void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte classBounsExpPercent, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            if ((int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("越级打怪开关")) == 0) {
            ...
            if (exp > 0) {
                ...
                if (attacker.hasDisease(MapleDisease.CURSE)) {
                    if (attacker.getEquippedFuMoMap().get(Integer.valueOf(32)) != null) {
                        exp *= 5;
                    }
                    else {
                        exp /= 2;
                ...
                if ((int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("网吧经验加成")) != 0) {
                    final int 网吧经验加成 = (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("网吧经验加成"));
                ...
                int wedding_EXP = 0;
                if (attacker.getMarriageId() > 0 && attacker.getMap().getCharacterById_InMap(attacker.getMarriageId()) != null && (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("结婚经验加成")) != 0) {
                    final int 结婚经验加成 = (int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("结婚经验加成"));
                    wedding_EXP = (int)((double)wedding_EXP + (double)exp / 100.0 * (double)结婚经验加成);
                }
                if ((int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("人气经验加成")) > 0 && attacker.getFame() > 0) {
                    attacker.人气经验加成();
                }
    ...
    public final void sendStatus(final MapleClient client) {
        ...
        if (this.poisons.size() > 0) {
            ...
            try {
                client.getSession().writeAndFlush(MobPacket.applyMonsterStatus(this, (List<MonsterStatusEffect>)this.poisons));
            }
            finally {
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if ((int)Integer.valueOf(FengYeDuan.ConfigValuesMap.get("怪物状态开关")) <= 0 && from.hasGmLevel(5)) {
            String 状态 = "";
        ...
        if (poison && this.getHp() > 1L) {
            ...
            if (from.hasGmLevel(5)) {
                from.dropMessage(6, "[持續伤害] 持續伤害： " + ((this.getHp() > (long)poisonDamage) ? ((long)poisonDamage) : (this.getHp() - 1L)) + " 持續时间：" + aniTime + " 持續掉血：" + status.getX());
            }
        }
    ...
    public final long getLastSkillUsed(final int skillId) {
        if (this.usedSkills.containsKey(Integer.valueOf(skillId))) {
            return (long)Long.valueOf(this.usedSkills.get(Integer.valueOf(skillId)));
        }
        return 0L;
    }
    ...
    private class PartyAttackerEntry implements AttackerEntry
    {
        ...
        @Override
        public void addDamage(final MapleCharacter from, final long damage, final boolean updateAttackTime) {
            final OnePartyAttacker oldPartyAttacker = (OnePartyAttacker)this.attackers.get(from.getId());
