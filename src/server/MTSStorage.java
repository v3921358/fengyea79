        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            for (final MTSItemInfo m : this.buyNow.values()) {
                if (now > m.getEndingDate()) {
                    ...
                    ((ArrayList<IItem>)expire.get(m.getCharacterId())).add(m.getItem());
                    ...
                    ((ArrayList<Pair<IItem, MapleInventoryType>>)items.get(m.getId())).add(new Pair<IItem, MapleInventoryType>(m.getItem(), GameConstants.getInventoryType(m.getItem().getItemId())));
                }
            }
        ...
        try {
            for (final Entry<Integer, MTSCart> c : this.idToCart.entrySet()) {
                ...
                if (expire.containsKey(c.getKey())) {
                    for (final IItem item : (ArrayList<IItem>)expire.get(c.getKey())) {
                        ((MTSCart)c.getValue()).addToInventory(item);
                    }
                }
