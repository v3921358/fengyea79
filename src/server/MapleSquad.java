...
public class MapleSquad
{
    ...
    public void copy() {
        while (((ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch))).size() > 0 && ChannelServer.getInstance(this.ch).getMapleSquad(this.type) == null) {
            int index = 0;
            long lowest = 0L;
            for (int i = 0; i < ((ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch))).size(); ++i) {
                if (lowest == 0L || (long)Long.valueOf(((Pair<String, Long>)((ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch))).get(i)).right) < lowest) {
                    index = i;
                    lowest = (long)Long.valueOf(((Pair<String, Long>)((ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch))).get(i)).right);
                }
            }
            final String nextPlayerId = (String)((Pair<String, Long>)((ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch))).remove(index)).left;
            ...
            if (theirCh > 0) {
                ...
                if (lead != null && lead.getMapId() == this.beginMapId && lead.getClient().getChannel() == this.ch) {
                    ...
                    if (ChannelServer.getInstance(this.ch).addMapleSquad(squad, this.type.name())) {
                        ...
                        ((ArrayList<Pair<String, String>>)this.type.queuedPlayers.get(Integer.valueOf(this.ch))).add(new Pair<String, String>(nextPlayerId, "成功"));
                        break;
                    }
                    squad.clear();
                    ((ArrayList<Pair<String, String>>)this.type.queuedPlayers.get(Integer.valueOf(this.ch))).add(new Pair<String, String>(nextPlayerId, "跳過"));
                    break;
                }
                else {
                    if (lead != null) {
                    ...
                    ((ArrayList<Pair<String, String>>)this.type.queuedPlayers.get(Integer.valueOf(this.ch))).add(new Pair<String, String>(nextPlayerId, "不在地图內"));
                }
            }
            else {
                this.getBeginMap().broadcastMessage(MaplePacketCreator.serverNotice(6, nextPlayerId + "'遠征队已經結束了，由於有成員沒有在線上"));
                ((ArrayList<Pair<String, String>>)this.type.queuedPlayers.get(Integer.valueOf(this.ch))).add(new Pair<String, String>(nextPlayerId, "沒有上線"));
            }
        }
    }
    ...
    public List<Pair<String, Long>> getAllNextPlayer() {
        return (ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch));
    }
    
    public String getNextPlayer() {
        final StringBuilder sb = new StringBuilder("\n排队成員 : ");
        sb.append("#b").append(((ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch))).size()).append(" #k ").append("與遠征队名單 : \n\r ");
        int i = 0;
        for (final Pair<String, Long> chr : (ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch))) {
            ++i;
    ...
    public void setNextPlayer(final String i) {
        ...
        for (final Pair<String, Long> s : (ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch))) {
        ...
        if (toRemove != null) {
            ((ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch))).remove(toRemove);
            return;
        }
        ...
        ((ArrayList<Pair<String, Long>>)this.type.queue.get(Integer.valueOf(this.ch))).add(new Pair<String, Long>(i, System.currentTimeMillis()));
    }
    ...
    public final Map<String, Integer> getJobs() {
        ...
        for (final Entry<String, String> chr : this.members.entrySet()) {
            if (jobs.containsKey(chr.getValue())) {
                jobs.put(chr.getValue(), Integer.valueOf((int)Integer.valueOf(jobs.get(chr.getValue())) + 1));
            }
            else {
