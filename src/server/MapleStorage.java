...
public class MapleStorage implements Serializable
{
    ...
    public byte getSlot(final MapleInventoryType type, final byte slot) {
        byte ret = 0;
        final List<IItem> it = (List<IItem>)this.typeItems.get(type);
    ...
    public void sendStored(final MapleClient c, final MapleInventoryType type) {
        c.sendPacket(MaplePacketCreator.storeStorage(this.slots, type, (Collection<IItem>)(List<IItem>)this.typeItems.get(type)));
    }
    
    public void sendTakenOut(final MapleClient c, final MapleInventoryType type) {
        c.sendPacket(MaplePacketCreator.takeOutStorage(this.slots, type, (Collection<IItem>)(List<IItem>)this.typeItems.get(type)));
    }
