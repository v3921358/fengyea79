...
public class MaplePvp
{
    ...
    private static void monsterBomb(final MapleCharacter player, final MapleCharacter attackedPlayers, final MapleMap map, final AttackInfo attack) {
        if (attackedPlayers.getLevel() > player.getLevel() + 1) {
            MaplePvp.pvpDamage /= 100;
        }
        else if (attackedPlayers.getLevel() < player.getLevel() - 1) {
            MaplePvp.pvpDamage /= 100;
        }
        else if (attackedPlayers.getLevel() == player.getLevel()) {
            MaplePvp.pvpDamage /= 100;
        }
        ...
        if (attackedPlayers.getStat().getHp() <= 0 && !attackedPlayers.isAlive()) {
            ...
            final int random = (int)Math.floor(Math.random() * 1.0E8);
            if (attackedPlayers.getMeso() >= random) {
                ...
                attackedPlayers.dropMessage("[系统警告] 您的金币已经不足，请马上离开。");
                player.dropMessage("[系统警告] 请不要再残害他，对方金币已耗尽。");
            }
        }
    }
