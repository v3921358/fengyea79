...
public class MapleQuest implements Serializable
{
    ...
    private static MapleQuest loadQuest(final ResultSet rs, final PreparedStatement psr, final PreparedStatement psa, final PreparedStatement pss, final PreparedStatement psq, final PreparedStatement psi, final PreparedStatement psp) throws SQLException {
        ...
        while (rse.next()) {
            ...
            ((List<Pair<String, Pair<String, Integer>>>)ret.partyQuestInfo.get(rse.getString("rank"))).add(new Pair<String, Pair<String, Integer>>(rse.getString("mode"), new Pair<String, Integer>(rse.getString("property"), rse.getInt("value"))));
        }
        rse.close();
        return ret;
    }
    ...
    public List<Pair<String, Pair<String, Integer>>> getInfoByRank(final String rank) {
        return (List<Pair<String, Pair<String, Integer>>>)this.partyQuestInfo.get(rank);
    }
    ...
    public static void initQuests() {
        ...
        try (final Connection con = (Connection)DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            while (rs.next()) {
                MapleQuest.quests.put(rs.getInt("questid"), loadQuest(rs, psr, psa, pss, psq, psi, psp));
            }
            rs.close();
            ps.close();
            psr.close();
    ...
    public static MapleQuest getInstance(final int id) {
        MapleQuest ret = (MapleQuest)MapleQuest.quests.get(Integer.valueOf(id));
        if (ret == null) {
