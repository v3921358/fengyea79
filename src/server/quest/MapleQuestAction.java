...
public class MapleQuestAction implements Serializable
{
    ...
    public MapleQuestAction(final MapleQuestActionType type, final ResultSet rse, final MapleQuest quest, final PreparedStatement pss, final PreparedStatement psq, final PreparedStatement psi) throws SQLException {
        ...
        switch (type) {
            ...
            case quest: {
                ...
                while (rs.next()) {
                    this.state.add(new Pair<Integer, Integer>(rs.getInt("quest"), rs.getInt("state")));
                }
                rs.close();
                break;
            }
            case skill: {
                ...
                while (rs.next()) {
                    this.skill.add(new Triple<Integer, Integer, Integer>(rs.getInt("skillid"), rs.getInt("skillLevel"), rs.getInt("masterLevel")));
                }
                rs.close();
                break;
            }
    ...
    public void runStart(final MapleCharacter c, final Integer extSelection) {
        ...
        switch (this.type) {
            ...
            case item: {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (item.prop > 0 && canGetItem(item, c)) {
                        for (int i = 0; i < item.prop; ++i) {
                            props.put(props.size(), Integer.valueOf(item.itemid));
                        }
                    }
                }
                int selection = 0;
                int extNum = 0;
                if (props.size() > 0) {
                    selection = (int)Integer.valueOf(props.get(Randomizer.nextInt(props.size())));
                }
            ...
            case nextQuest: {
                ...
                c.getClient().getSession().writeAndFlush(MaplePacketCreator.updateQuestFinish(this.quest.getId(), status.getNpc(), this.intStore));
                break;
            }
            case money: {
                final MapleQuestStatus status = c.getQuest(this.quest);
    ...
    public void runEnd(final MapleCharacter c, final Integer extSelection, final int qusetid) {
        ...
        switch (this.type) {
            ...
            case nextQuest: {
                c.getClient().getSession().writeAndFlush(MaplePacketCreator.updateQuestFinish(this.quest.getId(), c.getQuest(this.quest).getNpc(), this.intStore));
                break;
            }
            case money: {
                c.gainMeso(this.intStore, true, true);
