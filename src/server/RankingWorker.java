...
public class RankingWorker
{
    ...
    public final Integer getJobCommand(final String job) {
        return Integer.valueOf(this.jobCommands.get(job));
    }
    ...
    public final List<RankingInformation> getRankingInfo(final int job) {
        return (List<RankingInformation>)this.rankings.get(Integer.valueOf(job));
    }
    ...
    private void updateRanking(final Connection con) throws Exception {
        ...
        try (final PreparedStatement charSelect = con.prepareStatement(sb.toString());
            ...
            while (rs.next()) {
                ...
                final int jobRank = (int)Integer.valueOf(rankMap.get(Integer.valueOf(job / 100))) + 1;
                rankMap.put(Integer.valueOf(job / 100), Integer.valueOf(jobRank));
                ++rank;
                ((List<RankingInformation>)this.rankings.get(Integer.valueOf(-1))).add(new RankingInformation(rs.getString("name"), job, rs.getInt("level"), rs.getInt("exp"), rank));
                ((List<RankingInformation>)this.rankings.get(Integer.valueOf(job / 100))).add(new RankingInformation(rs.getString("name"), job, rs.getInt("level"), rs.getInt("exp"), jobRank));
                ps.setInt(1, jobRank);
