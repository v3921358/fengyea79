...
public class RankManager
{
    ...
    public List<MiniGamePoints> getTop(final int gameType, final int top) {
        ...
        if (!this.miniGamePointsMap.containsKey(Integer.valueOf(gameType))) {
            this.loadRank(gameType);
        }
        if (!this.miniGamePointsMap.containsKey(Integer.valueOf(gameType))) {
            return null;
        }
        if (this.topMap.containsKey(Integer.valueOf(gameType))) {
            return (List<MiniGamePoints>)this.topMap.get(Integer.valueOf(gameType));
        }
        ret = new ArrayList<MiniGamePoints>();
        int topIndex = 0;
        for (final MiniGamePoints mgp : (List<MiniGamePoints>)this.miniGamePointsMap.get(Integer.valueOf(gameType))) {
            if (topIndex >= 10) {
                break;
            }
