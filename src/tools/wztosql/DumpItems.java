    ...
    public void dumpItem(final PreparedStatement psa, final PreparedStatement psr, final PreparedStatement ps, final PreparedStatement pse, final MapleData iz) throws Exception {
        ...
        if (dat != null) {
            for (final MapleData child : dat) {
                ((Map<String, Integer>)equipStats.get(Integer.valueOf(-1))).put("mob" + MapleDataTool.getIntConvert("id", child, 0), MapleDataTool.getIntConvert("prob", child, 0));
            }
        }
            ...
            for (final MapleData info : dat) {
                ...
                for (final MapleData data2 : info) {
                    if (data2.getName().length() > 3 && data2.getType() != MapleDataType.CANVAS) {
                        ((Map<String, Integer>)equipStats.get(Integer.valueOf(lv))).put(data2.getName().substring(3), MapleDataTool.getIntConvert(data2, 0));
                    }
                }
            }
        }
        ...
        if (dat != null) {
            ...
            final Map<String, Integer> rett = (Map<String, Integer>)equipStats.get(Integer.valueOf(-1));
            ...
            for (final MapleData d2 : dat.getChildren()) {
                ...
                if (z != null) {
                    ...
                    if (z.isElement()) {
                        ...
                        incs = new Pair<Integer, Integer>(Element.getFromChar(ele.charAt(0)).getValue(), Integer.parseInt(ele.substring(1, ele.length())));
                    }
                    else {
                        incs = new Pair<Integer, Integer>(MapleDataTool.getIntConvert(z.getValue1(), d2, 0), MapleDataTool.getIntConvert(z.getValue2(), d2, 0));
                    }
